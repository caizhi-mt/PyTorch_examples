...
parser.add_argument('-d', '--device', default=None, type=str,
                    help='please specify the device as mtgpu if needed, default is None')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        #tmp_model = models.resnet50(num_classes=200)
    ...
    torch.save(model.to('cpu'), 'mtgpu_model_tiny_imagenet_90epoch.pth')
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        # TopK is not ready on mtGPU now
        output = output.to("cpu")
        _, pred = output.topk(maxk, 1, True, True)
        pred = pred.t()
        correct = pred.eq(target.view(1, -1).expand_as(pred).to("cpu"))
